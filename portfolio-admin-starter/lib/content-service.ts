...
export async function listContent(mode: ContentMode): Promise<ContentItem[]> {
  ...
  return mode === 'projects'
    ? (data as ProjectRow[]).map(projectFromRow)
    : (data as CertificateRow[]).map(certificateFromRow);
}
...
export async function saveContent(mode: ContentMode, item: ContentItem, exists: boolean): Promise<ContentItem> {
  if (mode === 'projects') {
    const payload = projectToRow(item as Project);
    const query = exists
  ...
  const payload = certificateToRow(item as Certificate);
  const query = exists
    ? supabase.from('certifications').update(payload).eq('id', item.id)
...
export async function deleteContent(mode: ContentMode, id: string) {
  const { error } = await supabase.from(tableForMode(mode)).delete().eq('id', id);
  if (error) throw error;
}

export async function countContent(mode: ContentMode) {
  const { count, error } = await supabase
    .from(tableForMode(mode))
...
function tableForMode(mode: ContentMode) {
  return mode === 'projects' ? 'projects' : 'certifications';
}
...
function certificateToRow(certificate: Certificate) {
  return {
    id: certificate.id,
    ...
    issued_at_label: certificate.issuedAtLabel,
